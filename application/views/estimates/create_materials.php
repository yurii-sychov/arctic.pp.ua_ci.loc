		...
		<form method="POST">
			...
			<div class="row my-2">
				<?php foreach ($materials as $k => $item) : ?>
					<div class="col-lg-4">
						<input type="checkbox" class="form-check-input" id="Id_<?php echo $item->id; ?>" name="material_id[]" value="<?php echo $item->id; ?>" <?php echo $item->checked ? 'checked disabled' : NULL; ?> />
						<label class="form-check-label" for="Id_<?php echo $item->id; ?>"><strong><?php echo $item->name; ?> (<?php echo $item->r3_id; ?>)</strong></label>
					</div>
				<?php endforeach; ?>
			</div>
			<div class="row my-2">
